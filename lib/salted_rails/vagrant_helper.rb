...
require "salted_rails/helper_base"

class SaltedRails < SaltedRailsBase
  class VagrantHelper < HelperBase

    # pass vm.ui for the logger if you want debugging info
    def initialize(rails_root, logger = Log4r::Logger.new("salted_rails::vagrant_helper"))
      super(rails_root, logger)
    end

    def configure_vagrant(config)
      @logger.debug 'Configuring vagrant' 
    ...
    def configure_ubuntu_mirror(config, mirror = 'mirror://mirrors.ubuntu.com/mirrors.txt')
      @logger.debug 'Configuring ubuntu mirror' 
    ...
    def configure_digital_ocean(config, private_key_path = '~/.ssh/id_rsa', disable_vagrant_sync = true)
      @logger.debug 'Configuring digital ocean provider' 
      @has_digital_ocean = 1
    ...
    def configure_salt(config)
      @logger.debug "Configuring saltstack"
      pillarize_application_configuration
      config.vm.synced_folder @rails_root + 'config/salt/', '/srv/salt/config/'
      config.vm.synced_folder @rails_root + 'config/pillar/', '/srv/pillar/config/'
      ...
      config.vm.provision :salt do |salt|
        @logger.debug 'Configuring salt provisioner'
    ...
    def configure_gui(vm_config, memory = 1024)
      @logger.debug "Configuring GUI with memory = %d" % memory
    ...
    def configure_memory(memory = 512)
      @logger.debug "Configuring memory = %d" % memory
